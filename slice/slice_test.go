...
import (
	...
	"github.com/wsva/lib_go/slice"
)

func TestCopyStrings(t *testing.T) {
	var src1 []string
	dest1 := slice.CopyStrings(src1)
...
func TestContainsString(t *testing.T) {
	src := []string{"aa", "bb", "cc"}
	if !slice.ContainsString(src, "bb", nil) {
	...
	if !slice.ContainsString(src, "ee", modifier) {
		t.Errorf("ContainsString didn't find the string by modifier")
	}

	src = make([]string, 0)
	if slice.ContainsString(src, "", nil) {
...
func TestRemoveString(t *testing.T) {
	...
	for _, tt := range tests {
		if got := slice.RemoveString(tt.input, tt.remove, tt.modifier); !reflect.DeepEqual(got, tt.want) {
